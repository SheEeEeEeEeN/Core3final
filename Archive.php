...
if (isset($_GET['restore'])) {
    ...
    if ($res && $res->num_rows > 0) {
        ...
        $activity = formatAdminActivityMessage("Restored document: " . $row['title']);
        $status = "Restored";
        $moduleEscaped = $conn->real_escape_string($module);
        $activityEscaped = $conn->real_escape_string($activity);
        $statusEscaped = $conn->real_escape_string($status);
        $conn->query("INSERT INTO admin_activity (`module`, `activity`, `status`, `date`) 
                    VALUES ('$moduleEscaped', '$activityEscaped', '$statusEscaped', NOW())");
...
<div class="sidebar flex-shrink-0 p-3" id="sidebar">
    ...
    <nav class="nav flex-column mb-auto mt-2">
        ...
        <div class="collapse show" id="archiveSubmenu" style="background: rgba(0,0,0,0.03); border-radius: 8px;">
            <a href="Archive.php" class="ps-5 active"><i class="bi bi-dot"></i> Documents</a>
...
<div class="content" id="mainContent">
    <header class="top-header d-flex align-items-center justify-content-between sticky-top mb-4 p-3 bg-white shadow-sm rounded-3">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h5 class="fw-bold mb-0 text-primary">Archived Documents</h5>
            </div>
        </div>
        <div class="d-flex align-items-center gap-3">
