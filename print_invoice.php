...
$currentStatus = isset($row['status']) ? strtoupper($row['status']) : '';

if (!empty($row['invoice_number']) && $currentStatus == 'DELIVERED') {
    $orNumber = $row['invoice_number'];
    $orColor = "#c0392b"; 
    $statusStamp = "PAID";
    $statusColor = "#d9534f"; 
} else {
    $orNumber = !empty($row['invoice_number']) ? $row['invoice_number'] : "PENDING";
    $orColor = "#7f8c8d"; 
    $statusStamp = "UNPAID";
    $statusColor = "#95a5a6"; 
}
