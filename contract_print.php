...
// Ensure this matches the '?number=' we added in the JS above
if (!isset($_GET['number'])) { 
    die("Error: Contract Number missing. Please select a route on the booking screen first."); 
}

$contract_number = mysqli_real_escape_string($conn, $_GET['number']);
// ... rest of your code

// 1. GET CONTRACT & CLIENT DETAILS using the contract_number string
...
if (!$contractQ || mysqli_num_rows($contractQ) == 0) {
    // Helpful debugging if contract isn't found
    die("Contract ($contract_number) not found in the database.");
}

$contract = mysqli_fetch_assoc($contractQ);
$contract_id = $contract['id']; // We get the ID here to use for the SLA rules query later
...
// ... rest of your display variables ...
...
<p>
The following Standard Trading Conditions shall form an integral part of this Agreement.
By signing this Agreement, the CLIENT confirms that they have read, understood, and agreed
to be bound by the Terms and Conditions stated herein.
</p>

<h3 class="clause-title"> Definitions</h3>
<p>
“Carrier” refers to Slate Freight and its authorized logistics partners (Core 2 Providers).
“Shipper” refers to the person or entity booking the shipment.
“Consignee” refers to the receiver of the goods.
</p>

<h3 class="clause-title"> Prohibited Items</h3>
<p>The CLIENT warrants that the shipment does <strong>NOT</strong> contain any of the following:</p>
<ul>
...
<p class="clause">
The PROVIDER reserves the right to inspect and refuse any shipment suspected of containing
prohibited items.
</p>

<h3 class="clause-title"> Shipper’s Responsibility (Packaging)</h3>
<p>
The CLIENT is solely responsible for proper packaging of goods. All items must be packed
to withstand the normal rigors of transportation. The PROVIDER shall <strong>NOT</strong> be liable
for damage caused by improper, inadequate, or insufficient packaging.
</p>

<h3 class="clause-title"> Limitation of Liability</h3>
<p>
Unless the CLIENT declares a higher value and pays the corresponding valuation or insurance
charges, the PROVIDER’s liability for loss or damage shall be limited to whichever is lower:
</p>
<ul>
    <li>The actual value of the goods; or</li>
...
including loss of profit due to delay.
</p>

<h3 class="clause-title"> Delivery Timeframe and Delays</h3>
<p>
Delivery dates generated by predictive systems or estimates are for reference only and are
<strong>NOT guaranteed</strong>. The PROVIDER shall not be liable for delays caused by traffic conditions,
checkpoint delays, weather conditions, or incorrect information supplied by the CLIENT.
</p>

<h3 class="clause-title"> Force Majeure</h3>
<p>
The PROVIDER shall not be held liable for any loss, damage, or delay caused by events beyond
its reasonable control, including but not limited to natural disasters, strikes, civil unrest,
or government actions.
</p>

<h3 class="clause-title"> Claims</h3>
<p>
Any claim for loss or damage must be filed within <strong>twenty-four (24) hours</strong> from the time of
delivery. Failure to report within this period shall constitute a waiver of any claim.
</p>
