...
if (isset($_POST['login']) && !$isLocked) {
    ...
    if ($id && password_verify($password, $db_password)) {
        ...
        $conn->query("DELETE FROM activity_log WHERE login_time < DATE_SUB(NOW(), INTERVAL {$activityRetentionDays} DAY)");
        $conn->query("DELETE FROM admin_activity WHERE `date` < DATE_SUB(NOW(), INTERVAL {$activityRetentionDays} DAY)");
