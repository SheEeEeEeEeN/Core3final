...
include_once __DIR__ . '/../activity_helpers.php';
requireRole('admin');
...
switch ($method) {
    ...
    case 'POST':
        ...
        if (move_uploaded_file($_FILES["uploadfile"]["tmp_name"], $targetFilePath)) {
            ...
            // Record activity
            $activity = $conn->real_escape_string(formatAdminActivityMessage("Uploaded document: $title"));
            $conn->query("INSERT INTO admin_activity (`module`,`activity`,`status`,`date`) 
                          VALUES ('E-Documentation','$activity','Pending Review',NOW())");
    ...
    case 'PUT':
        ...
        if ($id > 0) {
            ...
            if ($stmt->execute()) {
                $activity = $conn->real_escape_string(formatAdminActivityMessage("Edited document: $title"));
                $conn->query("INSERT INTO admin_activity (`module`,`activity`,`status`,`date`) 
                              VALUES ('E-Documentation','$activity','$status',NOW())");
    ...
    case 'DELETE':
        ...
        if ($id > 0) {
            ...
            if ($res && $res->num_rows > 0) {
                ...
                $activity = $conn->real_escape_string(formatAdminActivityMessage("Deleted document: " . $row['title']));
                $conn->query("INSERT INTO admin_activity (`module`,`activity`,`status`,`date`) 
                              VALUES ('E-Documentation','$activity','Deleted',NOW())");
