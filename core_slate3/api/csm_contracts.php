...
if ($method === 'POST') {
    ...
    if ($total >= $limit) {
        ...
        $activity = $conn->real_escape_string(formatAdminActivityMessage("Deleted oldest contract due to limit $limit"));
    ...
    if ($stmt->execute()) {
        $id = $stmt->insert_id;
        $act = formatAdminActivityMessage("Added new contract: $contract_id - $client_name");
...
if ($method === 'PUT') {
    ...
    if ($stmt->execute()) {
        $act = formatAdminActivityMessage("Updated contract ID $id");
        $conn->query("INSERT INTO admin_activity (module, activity, status) VALUES ('CSM', '". $conn->real_escape_string($act) ."', 'Success')");
...
if ($method === 'DELETE') {
    ...
    if ($conn->query("DELETE FROM csm WHERE id=$id")) {
        $act = formatAdminActivityMessage("Deleted contract: $name");
        $conn->query("INSERT INTO admin_activity (module, activity, status) VALUES ('CSM', '". $conn->real_escape_string($act) ."', 'Success')");
        echo json_encode(['success'=>true]);
    } else {
        http_response_code(500);
