...
<head>
  ...
  <style>
    ...
    body { font-family: 'Segoe UI', system-ui, sans-serif; background-color: #f8f9fc; color: var(--text-dark); }
    body.dark-mode { background-color: var(--dark-bg); color: var(--text-light); }

    /* Sidebar */
    .sidebar { width: 260px; height: 100vh; position: fixed; top: 0; left: 0; background: linear-gradient(180deg, #2c3e50 0%, #34495e 100%); color: white; z-index: 1000; transition: all .3s; }
    .sidebar.collapsed { transform: translateX(-100%); }
    .sidebar a { display: block; color: rgba(255, 255, 255, 0.8); padding: .75rem 1.5rem; text-decoration: none; border-left: 3px solid transparent; transition: .3s; }
    .sidebar a:hover, .sidebar a.active { background-color: rgba(255, 255, 255, 0.1); color: white; border-left: 3px solid white; }
    .content { margin-left: 250px; padding: 20px; transition: all .3s; }
    .content.expanded { margin-left: 0; }
    
    .header { background-color: white; padding: 1rem; border-radius: 0.5rem; box-shadow: var(--shadow); display: flex; align-items: center; justify-content: space-between; margin-bottom: 1.5rem; }
    
    /* Timeline CSS */
    .track-line { height: 4px; background-color: #e9ecef; width: 100%; position: absolute; top: 15px; z-index: 0; }
    .step-item { position: relative; z-index: 1; text-align: center; width: 33.33%; }
    .step-icon { width: 35px; height: 35px; background: #fff; border: 3px solid #e9ecef; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 10px; font-weight: bold; color: #adb5bd; transition: all 0.3s; }
    .step-item.active .step-icon { background: var(--primary-color); border-color: var(--primary-color); color: #fff; box-shadow: 0 0 10px rgba(78, 115, 223, 0.5); }
    .step-item.active .step-text { color: var(--primary-color); font-weight: 700; }
    .step-text { font-size: 0.8rem; color: #6c757d; }

    /* Dark Mode Overrides */
    body.dark-mode .header { background-color: var(--dark-card) !important; color: var(--text-light) !important; }
    body.dark-mode .card { background-color: var(--dark-card) !important; color: var(--text-light) !important; border-color: #2a3a5a !important; }
    body.dark-mode table, body.dark-mode tbody tr { background-color: var(--dark-card) !important; color: var(--text-light) !important; }
    body.dark-mode table tbody tr:hover { background-color: #2a3a5a !important; }
    body.dark-mode .modal-content { background-color: var(--dark-card); color: var(--text-light); border: 1px solid #2a3a5a; }
    body.dark-mode .bg-light { background-color: #243355 !important; color: white; border-color: #3a4b6e !important; }
    body.dark-mode .step-icon { background-color: #2c3e50; border-color: #495057; }
    body.dark-mode .track-line { background-color: #495057; }
  ...
  <div class="content" id="mainContent">
    <header class="header d-flex align-items-center justify-content-between px-4 py-3 mb-4 bg-white shadow-sm rounded-3 sticky-top">
      <div class="d-flex align-items-center gap-3">
        ...
        <div><h5 class="fw-semibold mb-0">Shipment History</h5></div>
      </div>
      <div class="d-flex align-items-center gap-3">
        <div class="dropdown">
          ...
          <div class="table-responsive">
            <table class="table align-middle table-hover">
              <thead class="table-primary">
                <tr>
                  <th>Tracking No</th>
                  <th>Destination</th> <th>Weight (kg)</th>
                  <th>Price (₱)</th>
                  <th>Payment</th>
                  <th>Status</th>
                  <th>Booked Date</th>
              ...
              <tbody id="historyTableBody">
                <?php if ($result->num_rows > 0): ?>
                  <?php while ($row = $result->fetch_assoc()): ?>
                    ...
                    $bg = match ($st) {
                      'Pending' => 'bg-warning', 'In Transit' => 'bg-info', 'Delivered' => 'bg-success', 'Cancelled' => 'bg-danger', default => 'bg-secondary'
                    };
...
<script>
    ...
    function openShipmentModal(id) {
        ...
        JsBarcode("#barcode", trackingNo, {
            format: "CODE128", lineColor: "#2c3e50", width: 2, height: 40, displayValue: false
        });

        const fd = new FormData();
        fd.append('action', 'get_shipment_details'); fd.append('id', id);
        ...
        .then(res => {
            if(res.success) {
                ...
                document.getElementById('modalPrice').textContent = "₱" + parseFloat(d.price).toLocaleString('en-US', {minimumFractionDigits: 2});
                ...
                if(st == 'cancelled') {
                    ...
                    let s = ''; for(let i=1; i<=5; i++) s += (i<=d.rating) ? '★' : '☆';
                    stars.innerHTML = s;
    ...
    async function updateStatus(id, st, r = null) {
        ...
        await fetch('update_shipment_api.php', { method: 'POST', body: fd });
        location.reload();
    }
