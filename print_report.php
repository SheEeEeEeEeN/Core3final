...
$totalSales = 0;
$generatedBy = $_SESSION['username'] ?? "Administrator"; // Pwede mong kunin sa session name mo
$dateGenerated = date('F j, Y h:i A');

include_once('activity_helpers.php');
$module = 'Reports';
$activityMsg = formatAdminActivityMessage("Generated $statusFilter shipment report from $startDate to $endDate");
$stmtLog = $conn->prepare("INSERT INTO admin_activity (`module`, `activity`, `status`, `date`) VALUES (?, ?, 'Success', NOW())");
$stmtLog->bind_param("ss", $module, $activityMsg);
$stmtLog->execute();
$stmtLog->close();
?>
<!DOCTYPE html>
