...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'sync_core1') {
            ...
            if ($foundStatus) {
                ...
                if (mysqli_query($conn, $sql)) {
                    $shipmentQuery = mysqli_query($conn, "SELECT * FROM shipments WHERE contract_number = '" . mysqli_real_escape_string($conn, $localContractNumber) . "' ORDER BY id DESC LIMIT 1");
                    if ($shipmentQuery && mysqli_num_rows($shipmentQuery) > 0) {
                        $shipment = mysqli_fetch_assoc($shipmentQuery);
                        $location = in_array(strtoupper($foundStatus), ['DELIVERED', 'OUT_FOR_DELIVERY', 'CANCELLED', 'ARCHIVED'], true)
                            ? ($shipment['destination_address'] ?? 'Destination hub')
                            : ($shipment['origin_address'] ?? 'Sorting hub');
                        logShipmentEvent(
                            $conn,
                            (int) $shipment['id'],
                            getShipmentTrackingNo($shipment),
                            strtoupper($foundStatus),
                            $location,
                            'Status synced from the dispatch network.'
                        );
                    }
        ...
        if ($action === 'update_status' && $_POST['status'] === 'Cancelled') {
            ...
            if (mysqli_query($conn, $sql)) {
                $shipmentResult = mysqli_query($conn, "SELECT * FROM shipments WHERE id = '$id' LIMIT 1");
                if ($shipmentResult && mysqli_num_rows($shipmentResult) > 0) {
                    $shipment = mysqli_fetch_assoc($shipmentResult);
                    logShipmentEvent(
                        $conn,
                        $id,
                        getShipmentTrackingNo($shipment),
                        ...
                        'Cancellation reason: ' . $_POST['reason']
                    );
                }
            ...
            if (mysqli_query($conn, $sql)) {
                ...
                if ($shipmentResult && mysqli_num_rows($shipmentResult) > 0) {
                    ...
                    logShipmentEvent(
                        ...
                        $shipment['destination_address'] ?? 'Destination',
                        !empty($proofPath)
                            ? 'Receiver confirmed delivery and uploaded proof of delivery.'
